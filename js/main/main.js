...
const {disconnect, color} = require("../../libs/echarts.common.min");
...
function bufferAnalysis(){
    ...
    console.log("市有" + allFeatures.length);
    ...
    for(let i = 0; i < turfFeatures.length; i++){
        ...
        if(!cross2){
            ...
            maybeDisaster(turfFeatures[i].geometry.coordinates[0],'rgba(255, 0, 0)');
            //alert("ok");
        }
    ...
    if (allFeatures) {
        ...
        console.log("没有找到名称为 '  ' 的图层中的多边形特征");
    }
    alert("运行完成");
}
...
async function nowData1() {
    ...
    xmlhttp.onreadystatechange=function()
    {
        if (xmlhttp.readyState==4 && xmlhttp.status==200)
        {
            //alert(this.responseText);
            //console.log(this.responseText);
            const bulletinString1 = JSON.parse(this.responseText);;

            //console.log(typeof bulletinString1);
            const typhoonData = bulletinString1[0]; //文本正则表达式转化为数组返回
           // alert("1");
            ...
            const initialPosition = {
                Number:typhoonData.subjective_forecast.ID,
                CNName:typhoonData.subjective_forecast.CNName || null,
                ENName:typhoonData.subjective_forecast.name,
                time:typhoonData.subjective_forecast.initial_time,
                lat: typhoonData.subjective_forecast.initial_position.lat,
                lng: typhoonData.subjective_forecast.initial_position.lng,
                pressure:typhoonData.subjective_forecast.initial_position.pressure,
                speed:typhoonData.subjective_forecast.initial_position.speed,
                moveSpeed: typhoonData.subjective_forecast.initial_position.moveSpeed
            };
            const positionsArray = [initialPosition];
            dataToTable(positionsArray); //加载基本信息表

            const arr = [initialPosition];
            bulletinString1.slice(1).forEach((position, index) => {
                const forecastPosition = {
                    time:position.subjective_forecast.initial_time,
                    lat: position.subjective_forecast.initial_position.lat,
                    lng: position.subjective_forecast.initial_position.lng,
                    pressure: position.subjective_forecast.initial_position.pressure, // 也可以包含其他信息
                    speed: position.subjective_forecast.initial_position.speed,
                    moveSpeed: position.subjective_forecast.initial_position.moveSpeed
                };
                arr.push(forecastPosition); // 将预测位置加入数组
                // 输出预测位置的信息
                //console.log(`过去位置经纬度: (${position.lat}, ${position.lng}) 气压:${position.pressure} 风速:${position.speed} 移速:${position.moveSpeed}`);
            });
            // 创建一个数组来存储所有位置的经纬度
             // 先将初始位置加入数组
            ...
            typhoonData.subjective_forecast.forecast_positions.forEach((position, index) => {
                ...
                positionsArray.push(forecastPosition); // 将预测位置加入数组
                // 输出预测位置的信息
                console.log(`预测时间 ${position.time_offset} 经纬度: (${position.lat}, ${position.lng}) 气压:${position.pressure} 风速:${position.speed}`);
            });
            // 现在 positionsArray 包含初始位置和所有预测位置的经纬度信息
            console.log(positionsArray);//
            const myArr = reverseEnds(arr);//数组倒置
            console.log(myArr);
            ...
            forecastData(positionsArray);//画预测路径
            //BufferRadius(positionsArray);

        }
    }
    //alert("正在查询");
    xmlhttp.open('GET', 'http://localhost:1100/api/data', true);
    xmlhttp.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
    xmlhttp.send();
}

function nowData2(url) {
    //未使用，nmc获取数据但没有实时的
    var xmlhttp;
    // alert("查询"+str);
    if (window.XMLHttpRequest)
    ...
    xmlhttp.onreadystatechange=function()
    {
        if (xmlhttp.readyState==4 && xmlhttp.status==200)
        {
            var typhoonData = JSON.parse(this.responseText);
            console.log(typhoonData.length);

            const positionsArray = [];

            typhoonData.track.forEach((position, index) => {
                const trackPosition = {
                    Time:position.time.slice(0, 16),
                    lat: parseFloat(position.lat),
                    lng: parseFloat(position.lon),
                    type: typhoonData.now.type,
                    pressure: parseInt(position.pressure,10),// 也可以包含其他信息
                    speed: parseInt(position.windSpeed,10),
                    moveSpeed: parseInt(position.moveSpeed,10)
                };
                positionsArray.push(trackPosition); // 将预测位置加入数组
                // 输出预测位置的信息
                console.log(` 经纬度: (${position.lat}, ${position.lon}) 气压:${position.pressure}  移速:${position.moveSpeed} 风速:${position.windSpeed}`);
            });
            const initialPosition = {
                // Number:typhoonData.subjective_forecast.ID,
                // CNName:typhoonData.subjective_forecast.CNName || null,
                // ENName:typhoonData.subjective_forecast.name,
                Time:typhoonData.now.pubTime.slice(0, 16),
                lat: parseFloat(typhoonData.now.lat),
                lng: parseFloat(typhoonData.now.lon),
                type: typhoonData.now.type,
                pressure:parseInt(typhoonData.now.pressure,10),
                speed:parseInt(typhoonData.now.windSpeed,10),
                moveSpeed: parseInt(typhoonData.now.moveSpeed,10),
            };
            positionsArray.push(initialPosition);
            console.log(positionsArray);
            const myArr = positionsArray;
    ...
    // alert("正在查询");
    //xmlhttp.open('GET', 'https://api.qweather.com/v7/tropical/storm-track?stormid=NP_2401&key=49fe07b7f3184dd3bd0b0e15adfda789', true);
...
function nowData3() {
    ...
    var str = 'https://api.qweather.com/v7/tropical/';
    var key = '49fe07b7f3184dd3bd0b0e15adfda789';
    var url = str + 'storm-list?basin=NP&year=' + year + '&key=' + key;
    // alert("查询"+str);
    if (window.XMLHttpRequest)
    {
        // IE7+, Firefox, Chrome, Opera, Safari 浏览器执行代码
    ...
    xmlhttp.onreadystatechange=function()
    {
        if (xmlhttp.readyState==4 && xmlhttp.status==200)
        {
            ...
            if(typhoonData.storm[0].isActive){
                const tyArr = {
                    id: typhoonData.storm[0].id,
                    name: typhoonData.storm[0].name,
                    year: typhoonData.storm[0].year,
                }
                const url1 = str + 'storm-track?stormid=' + tyArr.id + '&key=' + key;
                console.log(url1);
                nowData2(url1);
            }
